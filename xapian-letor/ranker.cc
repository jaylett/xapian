...
static map<Xapian::docid,double>
transform(Xapian::RankList &rl){

    map<Xapian::docid,double> blending_score;
...
static void
combineMap(map<Xapian::docid,double> & base, map<Xapian::docid,double> & new_add){
    if(base.size()!=new_add.size()){
        std::cout<<"the size of the ranking between each rankers are not the same"<<endl;
        exit(2);
    }
    else{
        for(map<Xapian::docid,double>::iterator iter = new_add.begin(); iter != new_add.end(); ++iter){
...
Ranker::aggregate(vector<Xapian::RankList> rls){
    ...
    if (0 == ranker_num){
        ...
        map<Xapian::docid,double> base = transform(rls[0]);
        for (int i = 1; i < ranker_num; ++i){
            map<Xapian::docid,double> new_add = transform(rls[i]);
